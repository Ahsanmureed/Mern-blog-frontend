...
const path=useLocation().pathname
  ...
  return (
    <nav className='py-5 px-[3px] md:px-7 flex items-center justify-between shadow-md w-screen fixed bg-white h-20 top-0 z-10'>
      ...
      <p onClick={()=>navigate(prompt?"?search="+prompt:navigate("/"))} className="    cursor-pointer"><BsSearch className=' text-[22px] '/></p>
        <input onChange={(e)=>setPrompt(e.target.value)} className=' md:ml-14 py-[6px] outline-none px-1 w-40 md:w-72 font-medium border rounded-md border-gray-200' type="text" name="" id="" placeholder='Search Blogs' />
      </div>
